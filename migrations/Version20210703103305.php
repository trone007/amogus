 ...
 */
final class Version20210703103305 extends AbstractMigration
{
    ...
    public function up(Schema $schema): void
    {
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__user AS SELECT id, ñ‚ñ„name, avatar, login FROM user');
        $this->addSql('DROP TABLE user');
        $this->addSql('CREATE TABLE user (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, avatar VARCHAR(511) DEFAULT NULL COLLATE BINARY, login VARCHAR(255) NOT NULL COLLATE BINARY, name VARCHAR(255) NOT NULL, updated_at DATETIME NOT NULL)');
        $this->addSql('INSERT INTO user (id, name, avatar, login) SELECT id, ñ‚ñ„name, avatar, login FROM __temp__user');
    ...
    public function down(Schema $schema): void
    {
        ...
        $this->addSql('CREATE INDEX IDX_F7C9CA858DB60186 ON round_task (task_id)');
        $this->addSql('CREATE INDEX IDX_F7C9CA8585ECDE76 ON round_task (completed_by_id)');
        $this->addSql('CREATE TEMPORARY TABLE __temp__user AS SELECT id, name, avatar, login FROM user');
        $this->addSql('DROP TABLE user');
        $this->addSql('CREATE TABLE user (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, avatar VARCHAR(511) DEFAULT NULL, login VARCHAR(255) NOT NULL, ñ‚ñ„name VARCHAR(255) NOT NULL COLLATE BINARY)');
        $this->addSql('INSERT INTO user (id, ñ‚ñ„name, avatar, login) SELECT id, name, avatar, login FROM __temp__user');
        $this->addSql('DROP TABLE __temp__user');
        $this->addSql('DROP INDEX IDX_272C116CA6005CA0');
        $this->addSql('DROP INDEX IDX_272C116CA76ED395');
        $this->addSql('CREATE TEMPORARY TABLE __temp__user_round AS SELECT id, round_id, user_id, role, status FROM user_round');
