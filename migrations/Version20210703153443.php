 ...
 */
final class Version20210703153443 extends AbstractMigration
{
    ...
    public function up(Schema $schema): void
    {
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__user AS SELECT id, avatar, login, name, updated_at, active FROM user');
        $this->addSql('DROP TABLE user');
        $this->addSql('CREATE TABLE user (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, login VARCHAR(255) NOT NULL COLLATE BINARY, name VARCHAR(255) NOT NULL COLLATE BINARY, updated_at DATETIME NOT NULL, active BOOLEAN NOT NULL, image VARCHAR(511) DEFAULT NULL)');
        $this->addSql('INSERT INTO user (id, image, login, name, updated_at, active) SELECT id, avatar, login, name, updated_at, active FROM __temp__user');
    ...
    public function down(Schema $schema): void
    {
        ...
        $this->addSql('CREATE TABLE user_tound (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL)');
        $this->addSql('DROP INDEX IDX_F7C9CA85A6005CA0');
        $this->addSql('DROP INDEX IDX_F7C9CA858DB60186');
        $this->addSql('DROP INDEX IDX_F7C9CA8585ECDE76');
        $this->addSql('CREATE TEMPORARY TABLE __temp__round_task AS SELECT id, round_id, task_id, completed_by_id, completed_at FROM round_task');
        ...
        $this->addSql('CREATE INDEX IDX_F7C9CA858DB60186 ON round_task (task_id)');
        $this->addSql('CREATE INDEX IDX_F7C9CA8585ECDE76 ON round_task (completed_by_id)');
        $this->addSql('CREATE TEMPORARY TABLE __temp__user AS SELECT id, name, image, login, updated_at, active FROM user');
        $this->addSql('DROP TABLE user');
        $this->addSql('CREATE TABLE user (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, name VARCHAR(255) NOT NULL, login VARCHAR(255) NOT NULL, updated_at DATETIME NOT NULL, active BOOLEAN NOT NULL, avatar VARCHAR(511) DEFAULT NULL COLLATE BINARY)');
        $this->addSql('INSERT INTO user (id, name, avatar, login, updated_at, active) SELECT id, name, image, login, updated_at, active FROM __temp__user');
        $this->addSql('DROP TABLE __temp__user');
        $this->addSql('DROP INDEX IDX_272C116CA6005CA0');
        $this->addSql('DROP INDEX IDX_272C116CA76ED395');
        $this->addSql('CREATE TEMPORARY TABLE __temp__user_round AS SELECT id, round_id, user_id, role, status FROM user_round');
